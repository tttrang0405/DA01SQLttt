...
(select * from 
(select t.* ,
row_number() over(partition by invoiceno, stockcode, quantity - phân cụm theo... order by invoicedate) as dup_flag
from online_retail_covert t ) x
where dup_flag =1 ) (đếm số lượng lớn hơn 1 để check dup)

bảng sau khi clean tên là online_retail_main
...
MIN(invoicedate) over(PARTITION BY customerid (--mỗi khách hàng)) as first_purchase_date ,
invoicedate -- ngày mua hàng hiện tại
